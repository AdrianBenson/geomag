...
CDIRS="internal/mseed internal/slink"
...
do
    ...
    docker run --rm \
        -e "GOBIN=/usr/src/go/src/github.com/${GH_USER}/${CWD}/${DOCKER_TMP}" -e "GOPATH=/usr/src/go" -e "GOFLAGS=-mod=vendor" -e "CGO_ENABLED=1" -e "GOOS=linux" -e "BUILD=$BUILD" \
        -v "$PWD":/usr/src/go/src/github.com/${GH_USER}/${CWD} \
        -w /usr/src/go/src/github.com/${GH_USER}/${CWD} --entrypoint /bin/sh ${BUILD_CONTAINER} \
        -c "apk add --no-cache gcc libc-dev make; for c in ${CDIRS}; do (cd \$c; make clean; make); done; go install -a -ldflags \"-X main.Prefix=${i}/${VERSION}\" -installsuffix cgo ./${i}"

    rm -f $DOCKER_TMP/Dockerfile

    echo "FROM alpine:3.10" > ${DOCKER_TMP}/Dockerfile
